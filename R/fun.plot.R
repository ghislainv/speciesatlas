...
fun.plot <- function(path,name,spdir,wcomp,p,zoom,enough,r.mar,e.map,BiomodData,BiomodModel,fut.var,npix,environ,s){
  ...
  if(enough){
    ...
    for (ind in 1:length(Index)) {
      v <- Find.Optim.Stat(Stat=Index[ind],Fit=caData,Obs=ObsData,Fixed.thresh=600) # ! here thresh=500: three models at least
      Perf.ca[,ind] <- v[1]
    }
    Perf.ca$Sen <- v[3]
    Perf.ca$Spe <- v[4]
